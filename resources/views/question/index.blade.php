        ...
        <div class="col-12">
            ...
            <nav class="breadcrumb-container d-none d-sm-block d-lg-inline-block" aria-label="breadcrumb">
                <ol class="breadcrumb pt-0">
                    ...
                    <li class="breadcrumb-item" aria-current="page"><a href="{{route('question.create')}}">جدید</a>
                    </li>
                ...
                <div class="card-body">
                    <table id="dataTable" class="table table-bordered table-hover">
                        ...
                        @foreach($questions as $question)
                            <tr>
                                <td><a href="{{route('question.show', $question->id)}}">{{$question->statement}}</a></td>
                                <td>{{$question->price}}</td>
                                <td><a class='fa-edit mx-1 fa text-primary h4' href="{{route('question.edit',$question->id)}}"></a>
                                    <form class='d-inline-block remove-button' action={{route('question.destroy',$question->id) }}   method='POST'>{{csrf_field()}} {{  method_field('DELETE')}}<button class="fa-trash mx-1 border-0 bg-transparent fa text-danger h4" type="submit"></button></form>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
